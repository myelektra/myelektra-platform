/**
 * inject-markers.mjs — v3 (simple, safe)
 *
 * Only replaces text content between HTML tags (between > and <).
 * Never touches attributes, URLs, or tag names.
 *
 * Usage:  node scripts/inject-markers.mjs
 */
...
// ─── Page → Config Path Mapping ────────────────────────────────────────
const PAGE_MAP = {
  'index.html': [
    'homepage.hero.badge', 'homepage.hero.headline', 'homepage.hero.headlineHighlight',
    'homepage.hero.subtitle', 'homepage.hero.ctaPrimary.label', 'homepage.hero.ctaPrimary.path',
    'homepage.hero.ctaSecondary.label', 'homepage.hero.ctaSecondary.path',
    ...
    'homepage.achievements.headline', 'homepage.achievements.items.0.title', 'homepage.achievements.items.0.description',
    ...
    'homepage.finalCta.ctaLabel', 'homepage.finalCta.ctaPath',
    'brand.tagline', 'brand.subTagline', 'brand.positioning',
    'navigation.headerCta', 'navigation.headerCtaPath',
    'global.floatingCta.text', 'global.floatingCta.buttonLabel', 'global.floatingCta.buttonPath',
...
/**
 * Replace text only between HTML tags.
 * Uses >text< pattern — never touches attributes or tag names.
 * Respects <script> and <style> blocks.
 */
function replaceInTextNodes(html, value, marker) {
  ...
  return blocks.map((block, i) => {
    // Only process blocks that aren't script/style
    if (i % 2 === 1) return block; // script/style block — leave untouched
    // Replace in text nodes only: text between > and <
    return block.replace(/>([^<]*?)</g, (match, text) => {
      if (text.includes(value)) {
        return '>' + text.replace(new RegExp(escaped, 'g'), marker) + '<';
      }
      return match;
    });
  }).join('');
...
function main() {
  ...
  for (const [file, pagePaths] of Object.entries(PAGE_MAP)) {
    ...
    for (const cp of pagePaths) {
      const val = flat[cp];
      if (!val) {
        console.warn(`  ⚠️  ${file}: config path '${cp}' not found`);
        continue;
      }

      const newHtml = replaceInTextNodes(html, val, `<!--@@${cp}@@-->`);
      if (newHtml !== html) {
        html = newHtml;
        fileReplaced++;
        totalReplaced++;
    ...
    console.log(`  ${file} — ${fileReplaced} markers injected`);
  }

  console.log(`\n✅ Done: ${totalReplaced} markers injected`);
  if (totalMissed > 0) {
    console.log(`⚠️  ${totalMissed} values not found in text nodes`);
  }
}
